...
#+begin_src emacs-lisp
(defvar pm/link-note-name-template "%<%s>__${url}__${slug}.org")
#+end_src
...
Capturing is the central component to how the note system should work. Ideally, the flow should work as a way to automate editing or creation. 
...
(cl-defun pm/note-capture-new (&key node)
  ...
  (if node
      (org-roam-capture-
       ...
       :info nil
       :keys nil
       :templates `(("a" "Basic note default" plain
                     ,pm/note-basic-entry
                    ...
                    ("d" "Basic note + tag prompt + alias prompt" plain
                     ...
                     :target (file+head
                              ,pm/default-note-name-template
                              ,(pm/template-head-builder :prompt-for-tags t)))
                     ("e" "Basic note + tag prompt + alias prompt" plain
                     ,pm/note-basic-entry
                     :target (file+head
                              ,pm/default-note-name-template
                              ,(pm/template-head-builder :prompt-for-tags t)))
...
(defvar pm/note-capture-existing-edit
  `("e" "Edit this file ${title}" plain
    ,pm/note-basic-entry
    :target (file+head ,pm/default-note-name-template ,(pm/template-head-builder))))

(defvar pm/note-capture-existing-todo
  `("t" "todo entry" entry
    ,pm/note-todo-entry
    :target (file+head+olp
             ,pm/default-note-name-template
             ,(pm/template-head-builder :headings `("Tasks"))
             ("Tasks"))))
...
(cl-defun pm/note-capture-existing (&key node)
  ...
  (if node
      (org-roam-capture-
       ...
       :info nil
       :keys nil
       :templates `(,pm/note-capture-existing-edit
                    ,pm/note-capture-existing-todo)
       :node node
       :props '(:unnarrowed t :empty-lines-before 1)) ;; 
    (user-error "Node cannot be nil!")))
