...
"
  (let ((alias (or (plist-get args :alias) nil))
    ...
    (let
        ((stringified-tags (apply #'concat
                                  `(":"
                                    ,(apply #'concat (pm/intersperse (cl-remove-if-not #'stringp tags) ":"))
                                    ":"
             ...
             (pm/intersperse
              (cl-remove-if
               ...
               `(,(when alias (concat ":PROPERTIES:\n:#+ROAM_ALIASES: " alias "\n:END:"))
                 ...
                 ,created
                 ,modified
                 ,(when stringified-tags (concat "#+filetags: " stringified-tags))
                 "\n"
...
(setq org-roam-dailies-capture-templates
      (let ((daily-note-name-template  "%<%Y-%m-%d>.org")
            (daily-note-head-template (pm/note-head-template
                                       :alias "%<%Y-%m-%d>"
        ...
        `(("d" "default" plain
           "%?"
           :target (file+head ,pm/daily-note-name-template ,daily-note-head-template)
           :unnarrowed t)
          ("j" "journal" item
           "- %<%T> :: %?"
           :target (file+head+olp ,pm/daily-note-name-template ,daily-note-head-template ("Journal"))
           :prepend t
           :unnarrowed t)
          ("t" "task" entry
           "** TODO %?\n:PROPERTIES:\n:CREATED:  %U\n:END:\n"
           :target (file+head+olp ,pm/daily-note-name-template ,daily-note-head-template ("Tasks"))
           :prepend t
           :unnarrowed t)
          ("i" "idea" entry
           "** IDEA %?\n:PROPERTIES:\n:CREATED:  %U\n:END:\n"
           :target (file+head+olp ,pm/daily-note-name-template ,daily-note-head-template ("Ideas"))
           :prepend t
           :unnarrowed t)
          ("l" "link" entry
           "** LINK %(org-cliplink-capture)\n:PROPERTIES:\n:CREATED:  %U\n:END:\n"
           :target (file+head+olp ,pm/daily-note-name-template ,daily-note-head-template ("Links"))
           :unnarrowed t
           :prepend t
           :immediate-finish t))
#+end_src
